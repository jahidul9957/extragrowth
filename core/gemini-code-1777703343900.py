...
# ==========================================
# 🤖 0. ULTIMATE MULTI-PLATFORM BOT ENGINE
# ==========================================
def run_bot_in_background(order_id):
    ...
    try:
        ...
        platform_name = order.service.platform.lower()
        service_name = order.service.name.lower()
        ...
        with sync_playwright() as p:
            ...
            for bot in bots:
                try:
                    ...
                    print(f"\n🚀 Bot [{bot.name}] is navigating to {platform_name.upper()}: {target_link}")
                    ...
                    if "youtube" in platform_name:
                        if "view" in service_name:
                            js_code = "() => { let vid = document.querySelector('video'); if(vid) { vid.play(); return 'CLICKED'; } return 'NOT_FOUND'; }"
                            result = page.evaluate(js_code)
                            if result == "CLICKED": time.sleep(35) 
                        else: 
                            js_code = """() => { const buttons = document.querySelectorAll('ytd-subscribe-button-renderer, yt-button-shape, button, div[role="button"]'); for(let b of buttons) { let text = (b.innerText || "").toLowerCase().trim(); if(text.includes('subscribed') || text.includes('सदस्यता ली')) return "ALREADY_SUBSCRIBED"; if(text === 'subscribe' || text.includes('सदस्यता लें')) { b.click(); return "CLICKED"; } } return "NOT_FOUND"; }"""
                            result = page.evaluate(js_code)

                    # 🟣 INSTAGRAM LOGIC
                    elif "instagram" in platform_name or "ig" in platform_name:
                        if "like" in service_name:
                            js_code = "() => { let unlike = document.querySelector('svg[aria-label=\"Unlike\"]'); if(unlike) return 'ALREADY_SUBSCRIBED'; let like = document.querySelector('svg[aria-label=\"Like\"]'); if(like) { like.closest('div[role=\"button\"]').click(); return 'CLICKED'; } return 'NOT_FOUND'; }"
                            result = page.evaluate(js_code)
                        elif "view" in service_name:
                            js_code = "() => { let vid = document.querySelector('video'); if(vid) { vid.play(); return 'CLICKED'; } return 'NOT_FOUND'; }"
                            result = page.evaluate(js_code)
                            if result == "CLICKED": time.sleep(20) 
                        elif "comment" in service_name:
                            try:
                                page.fill('textarea[aria-label="Add a comment…"]', "Awesome! 🔥")
                                page.click('div[role="button"]:has-text("Post")')
                                result = "CLICKED"
                            except:
                                result = "NOT_FOUND"
                    ...
                    if result == "ALREADY_SUBSCRIBED":
                        ...
                        print(f"📸 DANGER: Action failed for Bot [{bot.name}]")
